1. Converters:
   ...
   - YAML <-> JSON Converter: Useful for DevOps/Cloud configurations.
...
2. Generators:
   ...
   - QR Code Generator: Input text/URL -> Display QR code image (and allow save to disk).

3. Testers & Visualizers:
   - Regex Tester: Input a RegEx pattern and a test string -> Highlight matches in real-time.
   - Color Picker & Converter: A color wheel that gives you Hex, RGB, HSL, and CMYK values.
   - Unix Timestamp Converter: Human Date <-> Unix Timestamp (seconds/milliseconds).
   - Diff Viewer: Two text boxes (Left/Right) -> Highlight the differences between them.

Memo to myself: `flutter run -d windows` for testing app!
